...
def create_pair_csv(table, out, delimiter=";"):
    """
    @type table: dict[(int, int|None), dict]
    @type out: file
    ...
    csv_writer.writerow(("pid1", "qid1", "pid2", "count", "probability"))
    rowcount = 0
    for (pid1, qid1), row in table.iteritems():
        for pid2, value in row.iteritems():
            if pid1 != pid2 and isinstance(pid2, int) and value > 0:  # "appearances" and "type" is in the same table, ignore them
                ...
                if qid1 is None:
                    qid1 = 0
                csv_writer.writerow((pid1, qid1, pid2, value, probability))
                rowcount += 1
