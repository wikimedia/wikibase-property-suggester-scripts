...
def generate_random_integers(ids):
    ...
    while counter < 2 * sampleSize:
        ids.append(random.randint(0, 87))
        #ids.append(random.randint(0, 6656575))
        counter += 1


def handle_found_suggestion(rank):
...
if itemsOnFirst50Count > 0 and itemsOnFirst50PositionSum > 0:
    print "Average rank of extracted properties positioned within the first 50 suggestions: " + str(
        float(itemsOnFirst50PositionSum) / itemsOnFirst50Count)
