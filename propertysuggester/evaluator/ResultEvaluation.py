...
class ResultEvaluation:
    ...
    def generate_random_integers(self):
        ...
        while counter < 2 * self.samplesize:
            self.random_ids.append(random.randint(0, 6656575))
            counter += 1
    ...
    def determine_ranks(self):
        with open(self.inputfile, "r") as f:
            ...
            for entity in CsvReader.read_csv(f):
                if not self.foundEntities >= self.samplesize:
                    if current_item_number in self.random_ids and len(entity.claims) > 1:
                        self.process_entities(entity)
                current_item_number += 1
        f.close()
        return current_item_number
